...
- `ml` (FastAPI + Nomeroff-Net) читает видео, детектит номера и отправляет raw/confirmed события.
...
- `ML_FORCE_RU_OCR=true`: ML фиксирует OCR на `ru` и отключает авто-классификацию региона, это уменьшает ошибки для российских номеров.
...
- Для Docker `ml` использует локальный кэш моделей из `services/ml/venv/Lib/site-packages/data` (через `LOCAL_STORAGE`), чтобы не тянуть модели заново при старте.
